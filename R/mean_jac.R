mean_jac <- function(
  ...
  if (!is.null(params$alpha_tilde)) {
    Z_tilde <- construct_Z_tilde(Z_tilde_list, params$alpha_tilde)
  }
  ...
  param_status <- dataframes_to_parameters(fixed_status, varying_status)

  n <- nrow(X)
  J <- ncol(params$P)
  n_varying <- nrow(varying_df)

  jacobian <- matrix(nrow = n * J, ncol = n_varying)
  counter <- 0
  for (i in 1:n) {
    for (j in 1:J) {
      counter <- counter + 1
