...
simpl_auglag_fnnls <- function(
  ...
  for (k in 1:maxit) {
    ...
    ATA <- 0.5 *
      xhess +
      lambda * diag(rep(1, length(xgrad))) +
      matrix(mu, nrow = npar, ncol = npar)

    Ab <- t(
      (-0.5) *
        (matrix(nu - 2 * mu, nrow = 1, ncol = npar) -
          2 * lambda * t(x0) -
          t(x0) %*% xhess +
          t(xgrad))
    )
    ...
    if (counter >= 60) {
      warning(paste(
        "In fastNNLS, the counter is",
        counter,
        "which is >=60. Potential convergence issue?"
      ))
    }
    ## END Amy July 8 2023

    x <- fast_nnls(ZTx = Ab, ZTZ = ATA, tolerance = constraint_tolerance)
