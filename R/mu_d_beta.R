...
#' @param q Which row of B to take derivative with respect to
#' (must be in 1, ..., p)
...
#'
#' @return A derivative d mu_ij / d B_qj
  ...
  for(k_tilde in 1:K_tilde){
    if(k_tilde %in% Z_tilde_gamma_cols){
      ...
      mu_deriv <- mu_deriv + exp(gammas[i])*
        Z_tilde[i,k_tilde]%*%(P_tilde[k_tilde,j] *
                                exp(gamma_tilde[k_tilde]  +
                                      X_tilde[k_tilde,]%*%B[,j]))*X_tilde[k_tilde,q]
