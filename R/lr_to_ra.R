lr_to_ra <- function(fixed_df, varying_lr_df, varying_df) {
  ...
  for (rho_row in rho_rows) {
    ref_j <- max(varying_df$j[
      varying_df$param == "P" & varying_df$k == rho_row
    ])
    ...
    scaling <- 1 -
      sum(fixed_df$value[(fixed_df$k == rho_row) & (fixed_df$param == "rho")])
  ...
  for (rho_tilde_row in rho_tilde_rows) {
    ...
    unscaled_ra <- exp(
      c(rho_tilde_mini$value, 0) - sum_of_logs(c(rho_tilde_mini$value, 0))
    )

    scaling <- 1 -
      sum(fixed_df$value[
        (fixed_df$k == rho_tilde_row) &
  ...
  P_tilde_df <- P_tilde_df[order(P_tilde_df$k, P_tilde_df$j), ]
  ...
  to_return_df <- rbind(
    to_return_df[to_return_df$param == "P", ],
