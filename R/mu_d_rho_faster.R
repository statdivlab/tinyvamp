...
#' @param proportion_scale No idea
#'
#' @return A derivative d mu_i / d rho_k
mu_d_rho_faster <- function(
  i,
  J,
  k,
  gammas,
  ...
  dmu_i_dpk <- Matrix::Diagonal(
    x = as.numeric(
      Z[i, k] *
        exp(matrix(gammas[i], nrow = 1, ncol = J) + X[i, , drop = F] %*% B)
    )
  )
  if (!proportion_scale) {
    nu_k <- exp(rho_k)
    front_term <- 1 / (1 + sum(nu_k))
    dpk_drhok <- Matrix::Matrix(
      fixed_P_multipliers[k] *
        (front_term *
          rbind(diag(nu_k), 0) -
