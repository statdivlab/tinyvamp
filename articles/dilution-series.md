...
Now we can construct the count data matrix $\mathbf{W}$. We will reorder
its columns so the taxa in the mock come last, and reorder rows so
dilutions are in increasing order – this just makes our lives easier
later!
...
- we are not interested in estimating detection efficiencies for
  contaminants (this is $\widetilde{\mathbf{X}}$)
...
- the contamination relative abundance profile
  ($\widetilde{\mathbf{p}}$)
- the detection efficiencies for mock taxa ($\mathbf{β}$)
...
$\mathbf{Z}$ might look like $\begin{pmatrix}
1 & 0 \\
1 & 0 \\
1 & 0 \\
0 & 1 \\
...
“keep $\mathbf{p}$ fixed at the values that we tell you; you don’t need
to estimate it yourself!”

In contrast, we do need to estimate *some* of the $\beta$’s, a.k.a. the
detection efficiencies. We want to estimate the detection efficiencies
of the mock taxa, but not for the contaminants.
...
Hang on, why is $\beta_{248}$ fixed? Well, we always need some taxon to
compare detection efficiencies to – otherwise there’s no difference
...
$\mathbf{X}$ connects the samples ($n$ rows) to the sequencing protocols
($p$ columns). Here we have only one protocol (no data on batches, for
example), so one column. We’ll just fill this matrix with 1’s to say
“estimate efficiencies for our single protocol”.
...
community, you might set $\begin{pmatrix}
1 & 0 \\
\vdots & \vdots \\
1 & 0 \\
0 & 1 \\
...
so $\widetilde{K} = 1$ and the number of columns of
$\widetilde{\mathbf{Z}}$ is one. We want to assume that the ratio of
expected contaminant reads to expected mock reads is proportional to
$3^{d}$, where $d$ is the dilution number of the sample ($d_{i} = 0$
corresponds to the undiluted sample). To do this, we set
${\widetilde{\mathbf{Z}}}_{i}$ proportional to
$\exp\left( \gamma_{i} \right) \times 3^{d_{i}}$ for sample $i$. To
address the $3^{d_{i}}$ piece, we set

``` r
Z_tilde <- matrix(3^(0:8)/exp(mean(log(3^(0:8)))), ncol = 1)
```

and to address the $\exp\left( \gamma_{i} \right)$ piece, we set
...
the contaminants, so we set $\widetilde{\mathbf{X}} = \mathbf{0}$:
