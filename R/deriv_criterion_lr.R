...
deriv_criterion_lr <- function(
  ...
  log_P_tilde <- lapply(1:K_tilde, function(k) {
  ...
  if (include_log_penalty_derivatives) {
    lp_grad <- rep(
      0,
      sum(!(varying_lr_df$param %in% c("rho", "rho_tilde", "alpha_tilde")))
    )
    if (return_info) {
    ...
    for (k in which_rho_k) {
      ...
      if (return_info) {
        ...
        new_hess_piece <- cbind(
          matrix(0, nrow = new_rows, ncol = hess_rows),
          new_hess_piece
        )
        lp_hess <- cbind(lp_hess, matrix(0, nrow = hess_rows, ncol = new_rows))
        lp_hess <- rbind(lp_hess, new_hess_piece)
      }
    }

    for (k in which_rho_tilde_k) {
      temp_rho_tilde <- varying_lr_df$value[
        varying_lr_df$param == "rho_tilde" &
          varying_lr_df$k == k
      ]
  ...
  if (n_alpha_tilde > 0) {
    if (include_log_penalty_derivatives) {
      lp_grad <- c(lp_grad, rep(0, n_alpha_tilde))
    }

    if (return_info) {
      lp_hess <- cbind(
        lp_hess,
        matrix(0, ncol = n_alpha_tilde, nrow = nrow(lp_hess))
      )

      lp_hess <- rbind(
        lp_hess,
        matrix(0, nrow = n_alpha_tilde, ncol = ncol(lp_hess))
      )
    }
  }
  if (!return_info) {
  ...
  if (!include_log_penalty_derivatives) {
    return(list("grad" = crit_grad, "info" = crit_info))
  } else {
    return(list("grad" = crit_grad + lp_grad, "info" = crit_info + lp_hess))
  }
}
