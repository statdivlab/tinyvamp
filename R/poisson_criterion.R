poisson_criterion <- function(W, means, wts = NULL) {
  n <- nrow(W)
  J <- ncol(W)
  wt_list <- lapply(1:n, function(i) wts[1:J + (i - 1) * J])
  ...
  if (is.null(wts)) {
    return(sum(sapply(1:nrow(means), function(i) {
      sapply(1:ncol(means), function(j) {
