get_A_tilde_k_list <- function(fixed_df, varying_df, varying_lr_df) {
  ...
  for (k in which_k_p_tilde) {
    fixed_p_tilde_k <- fixed_df[
      (fixed_df$param == "P_tilde") & (fixed_df$k == k),
    ]
    varying_p_tilde_k <- varying_df[
    ...
    A_tilde_k <- matrix(0, nrow = J, ncol = C_k)
