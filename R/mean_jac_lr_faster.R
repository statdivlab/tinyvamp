...
mean_jac_lr_faster <- function(
  ...
  if (proportion_scale) {
    if (
      is.null(P_fixed_indices) |
        is.null(P_tilde_fixed_indices)
    ) {
      stop(
        "P_fixed_indices and P_tilde_fixed_indices must be provided if
proportion_scale = TRUE"
  ...
  jacobian <- matrix(0, nrow = n * J, ncol = n_varying)

  if (!proportion_scale) {
    npars_rho <- sapply(1:length(Ak_list), function(k) {
      if (is.null(Ak_list[[k]])) {
        0
      } else {
        dim(Ak_list[[k]])[2] - 1
      }
    ...
    npars_rho_tilde <- sapply(1:length(A_tilde_k_list), function(k) {
      if (is.null(A_tilde_k_list[[k]])) {
        0
      } else {
    ...
    npars_rho <- sapply(1:K_max, function(d) {
      sum(
        varying_df$k[varying_df$param == "P"] == d
      )
    })

    npars_rho_tilde <- sapply(1:K_tilde_max, function(d) {
      sum(
  ...
  for (i in 1:n) {
    ...
    if (sum(npars_B) > 0) {
      ...
      which_B_local <- which(
        (X[i, ] != 0) |
          apply(X_tilde[Z_tilde[i, ] > 0, , drop = F], 2, function(xtil) {
            as.logical(sum(xtil != 0))
          })
      )
      ...
      which_B_local <- sapply(which_B_local, function(x) sum(which_B_rows <= x))
      ...
      if (length(which_B_local) > 0) {
        for (B_row_index in which_B_local) {
          jacobian[
            ...
            diag(sapply(1:J, function(j) {
              ifelse(
                j < J,
                ifelse(
                  which_B_keep[B_row_index, j],
                  mu_d_beta(
                    i = i,
                    j = j,
                    q = B_row_index,
            ...
            }))[, which(which_B_keep[B_row_index, ])]
    ...
    if (n_gammas > 0) {
      ...
      if (i %in% which_gammas) {
        jacobian[(i - 1) * J + 1:J, sum(npars_B) + sum(which_gammas <= i)] <-
          mu_d_gamma_faster(
            i,
            J,
            gammas = params$gammas,
      ...
      for (rho_index in which_rho_local) {
        if (!proportion_scale) {
          jacobian[
            ...
            sum(npars_B) +
              n_gammas +
              K_gamma_tilde_effective +
              sum(npars_rho[which_k_p[which_k_p < rho_index]]) +
              1:(npars_rho[rho_index])
          ] <-
            as.matrix(mu_d_rho_faster(
              i,
              J,
              gammas = params$gammas,
              B = params$B,
              ...
              k = rho_index,
              Ak_list = Ak_list,
              rho_k = varying_lr_df$value[
                varying_lr_df$k == rho_index &
          ...
          jacobian[
            ...
            sum(npars_B) +
              ...
              ifelse(
                rho_tilde_index == 1,
                0,
                sum(sapply(1:(rho_tilde_index - 1), function(w) {
                  npars_rho_tilde[[w]]
                }))
              ) +
              1:npars_rho_tilde[[rho_tilde_index]]
          ] <-
            as.matrix(mu_d_rho_tilde_faster(
              i = i,
              J = J,
              gammas = params$gammas,
              B = params$B,
              rho_tilde_k = varying_lr_df$value[
