parameters_to_dataframes <- function(
  P,
  P_fixed_indices,
  P_tilde,
  ...
  if (length(c(J, K, K_tilde, p_B, n)) < 5) {
    ...
    stop(c(
      "Unfortunately, one of J, K, K_tilde, p_B or n is NULL.\n",
  ...
  fixed_df <- rbind(fixed_df, known_temp_df)

  varying_df <- rbind(varying_df, varying_temp_df)
  ...
  known_temp_df <- data.frame(
    "value" = P_tilde[P_tilde_fixed_indices],
  ...
  varying_temp_df <- varying_temp_df[
    order(varying_temp_df$k, varying_temp_df$j),
  ]
  ...
  fixed_df <- rbind(
    fixed_df,
    temp_df[((1:n) %in% which(gammas_fixed_indices)), ]
  )
  ...
  fixed_df <- rbind(
    fixed_df,
    temp_df[((1:K_tilde) %in% which(gamma_tilde_fixed_indices)), ]
  )
  ...
  return(list("fixed" = fixed_df, "varying" = varying_df))
}
