...
#' @param fixed_P_tilde_multipliers Numeric vector of row-specific multipliers
#'   applied to the derivative of the transformation from
#'   \eqn{\tilde{\rho}} to \eqn{\tilde{P}}.
...
#' @param gamma_tilde Numeric vector of row-specific intercept terms for the
#'   spurious read model.
#' @param proportion_scale Logical. If \code{FALSE}, return the derivative of
#'   \eqn{\mu_i} with respect to row \code{k_tilde} of \eqn{\tilde{\rho}}. If
#'   \code{TRUE}, return only the derivative with respect to the corresponding
#'   row of \eqn{\tilde{P}}.
#'
#' @return A derivative d mu_i / d rho_tilde_k
mu_d_rho_tilde_faster <- function(
  i,
  J,
  k_tilde,
  gammas,
  ...
  dmu_i_dptildektilde <- Matrix::Diagonal(
    x = as.numeric(
      Z_tilde[i, k_tilde] *
        exp(
          matrix(gamma_tilde[k_tilde], nrow = 1, ncol = J) +
  ...
  if (!proportion_scale) {
    ...
    dptildek_drhotildek <- Matrix::Matrix(
      fixed_P_tilde_multipliers[k_tilde] *
        (front_term *
          rbind(diag(nu_tilde_k), 0) -
          front_term^2 * outer(c(nu_tilde_k, 1), nu_tilde_k))
    )

    rho_tilde_jacob_i <- dmu_i_dptildektilde %*%
      A_tilde_k_list[[k_tilde]] %*%
      dptildek_drhotildek
