do_one_simulation <- function(
  ...
  reweighted_fit <- try(fit_simulation_model(
    W,
    "reweighted_Poisson",
  ...
  if (is.list(reweighted_fit)) {
    ...
    reweighted_ci <- try(bootstrap_ci(
      W = W,
      fitted_model = reweighted_fit,
      n_boot = n_boot,
      alpha = 0.05,
