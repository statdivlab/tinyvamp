fit_simulation_model <- function(W, estimator, return_variance = FALSE) {
  n <- nrow(W) / 16
  J <- ncol(W)

  Z_tilde <- do.call(
    rbind,
    lapply(1:n, function(x) matrix(rep(c(1, 9, 81, 729), 4), ncol = 1))
  )
  ...
  fitted_model <- estimate_parameters(
    W = W,
    X = X,
    Z = Z,
