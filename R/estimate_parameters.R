...
#' @param initial_conv_tol Are we flat enough in the likelihood yet? Where to start.
#' @param final_conv_tol Are we flat enough in the likelihood yet? Where to end.
#' @param constraint_tolerance The tolerance for the augmented Lagrangian algorithm. Final
#' estimates of P are relative abundances to within \code{constraint_tolerance} of 1, i.e., \eqn{abs(sum p_{kj} - 1) <}  \code{constraint_tolerance}. Defaults to 1e-10.
...
estimate_parameters <- function(
  ...
  if (!(criterion %in% c("Poisson", "reweighted_Poisson"))) {
    stop(
      "Argument `criterion` must be equal to
`Poisson` or `reweighted_Poisson`."
  ...
  if (sum(P[!P_fixed_indices] == 0) > 0) {
    if (verbose) {
      message("huh, Amy isn't sure why this line gets hit...")
    }
  ...
  if (!is.null(alpha_tilde)) {
    if (is.null(Z_tilde_list)) {
      stop(
        "If alpha_tilde is included in model, components of Z_tilde
...
Z_tilde_list"
  ...
  if (is.null(Z_tilde)) {
    ...
    if (!is.null(Z_tilde_list)) {
      warning(
        "Z_tilde argument is ignored when Z_tilde_list is provided.
...
alpha_tilde and matrices in Z_tilde_list.)"
  ...
  if (criterion == "GMM") {
    if (verbose) {
      message("Fitting initial Poisson estimator")
    }
    poisson_fit <-
      estimate_parameters(
        W = W,
    ...
    poisson_fit_params <- dataframes_to_parameters(
      fixed_df = poisson_fit$fixed,
  ...
  if (criterion == "reweighted_Poisson") {
    ...
    poisson_means <- meaninate(
      gammas = poisson_fit$gammas,
      B = poisson_fit$B,
      X = X,
      Z = Z,
      P = P,
    ...
    pre_wts <- tryCatch(
      #cir::cirPAVA(y = squerror_long,
      # x = means_long,
      # wt= wts),
      cir::cirPAVA(
        y = squerror_long[order(means_long)],
        x = means_long[order(means_long)],
        wt = wts[order(means_long)]
      )[rank(means_long)],
      error = function(c) {
        if (verbose) {
          message(
            "Fitted means are the same, breaking cirPAVA...\nNot to worry! Jitter and refit...\n"
          )
        }
      }
    )
  ...
  parameter_dfs <- parameters_to_dataframes(
    P,
    P_fixed_indices,
    P_tilde,
    P_tilde_fixed_indices,
  ...
  Ak_list <- get_Ak_list(fixed_df, varying_df, varying_lr_df)

  A_tilde_k_list <- get_A_tilde_k_list(fixed_df, varying_df, varying_lr_df)

  # message("created Ak_list and A_tilde_k_list")

  #calculate at outset of optimization  pass as argument to mean_jac_lr, etc.
  which_k_p <- sapply(1:K, function(k) ifelse(is.null(Ak_list[[k]]), NA, k))

  which_k_p <- which_k_p[!is.na(which_k_p)]

  #calculate at outset of optimization  pass as argument to mean_jac_lr, etc.
  which_k_p_tilde <- sapply(1:K_tilde, function(k) {
    ifelse(
  ...
  #calculate at outset of optimization
  which_B_keep <- lapply(which_B_rows, function(k) {
  ...
  which_unconstrained <- varying_lr_df$param %in%
    c("B", "gamma", "gamma_tilde", "alpha_tilde")
  ...
  while ((barrier_t <= max_barrier) & (barrier_counter <= barrier_maxit)) {
    ...
    while (sqrt(sum(derivs$grad^2)) > tolerances[newton_counter]) {
      ...
      if (verbose) {
        message("Calculating step direction...")
      }

      step_direction <- qr.solve(
        derivs$info +
          unconstrained_regularization *
      ...
      if (is.matrix(step_direction)) {
        ...
        grad_amount <-
          as.numeric(
            -matrix(as.numeric(derivs$grad), nrow = 1) %*%
              step_direction
          )
      ...
      if (stepsize < 1e-5) {
        ...
        hessian_regularization <- max(
          hessian_regularization / 2,
          min_regularization
        )
      }
      ...
      if (verbose) {
        ...
        message(paste(
          "  Sum of squared gradients is ",
          sqrt(sum(derivs$grad^2))
        ))
      }
    }
    ...
    if (verbose) {
      message(paste(
        "Fit barrier sub-problem with t = ",
        barrier_t,
        ".",
        sep = "",
        collapse = ""
  ...
  temp_params <- dataframes_to_parameters(fixed_df, varying_df)
  ...
  if (profile_P) {
    ...
    if (length(which_k_p) > 0) {
      while (
        (old_crit_value - crit_value > 1e-4) &
          (profile_counter <= profiling_maxit)
      ) {
        if (verbose) {
          message(paste("  Profiling counter", profile_counter))
        }
        ...
        for (k in which_k_p) {
          ...
          p_hess <- t(jacobian) %*%
            ...
            jacobian +
            diag(rep(
              hessian_regularization * sqrt(sum(p_grad^2)),
              nrow(p_grad)
            ))
          ...
          temp_dfs$varying$value <-
            simpl_opt_linesearch_fnnls(
              fn = temp_fn, #objective function to be minimized
          ...
          temp_params <- dataframes_to_parameters(
            fixed_df = temp_dfs$fixed,
            varying_df = temp_dfs$varying
          )
  ...
  if (!is.null(archived_varying)) {
    final_dfs$varying <- rbind(final_dfs$varying, archived_varying)
